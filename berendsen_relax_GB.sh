...
if [ $verbose = true ]; then
    lmp_omp_edited -in in.berendsen_relax -var gbname $name -var structure_name $structure -pk omp ${job} -sf omp
else
    log=$(lmp_omp_edited -in in.berendsen_relax -var gbname $name -var structure_name $structure -pk omp ${job} -sf omp)
fi
fi
echo; echo "LAMMPS task done, plotting..."; echo

mkdir ../GB_projects/$name/images
python plot_thermal_relax.py --name $name -m $mean_width --inp berendsen_relax
cd ..
